...
async function main() {
  for (const item of allProjectsToTest) {
    ...
    process.stdout.write(`[${item.tag}] ${p.name} (/du-an/${p.slug}) | WS:${p.website ? p.website.status : 'AUTO-FALLBACK'} | Inv:${p.inventories.length} List:${p.listings.length} Res:${p.resources.length} 360:${!!tour360} | Active(${activeSections.length}): ${activeSections.join(',')} | Hidden: ${hiddenSections.join(',') || 'None'}\n`);
  }
}

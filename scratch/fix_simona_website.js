...
async function fixSimonaWebsite() {
  console.log("=== FIXING SIMONA HEIGHTS WEBSITE & RESOURCES ===");

  // Find Simona Heights project
  ...
  if (!project) {
    console.error("❌ Project Simona Heights not found in database!");
    return;
  }

  console.log("✅ Found Simona project:", project.id, project.name, project.slug);

  // Sample Sections Config
  ...
  // Sample Content JSON for Simona Heights
  const contentJson = {
    hero: {
      ...
      tagLine: "🏨 Căn Hộ Hạng Sang Trung Tâm Quy Nhơn",
    ...
    location: {
      address: "145 Trần Hưng Đạo, P. Trần Hưng Đạo, TP. Quy Nhơn, Bình Định",
  ...
  // 1. Create or update ProjectWebsite for Simona Heights
  ...
  console.log("✅ Seeded & Published ProjectWebsite:", website.id);

  // 2. Update ProjectResource so Mở website button in /tai-lieu-du-an/website points directly to /du-an/simona-heights-quy-nhon
  const targetMicrositeUrl = `/du-an/${project.slug}`;

  // Find existing website resource or create
  ...
  if (existingResource) {
    await prisma.projectResource.update({
      ...
      data: {
        ...
        description: "Trang thông tin giới thiệu, tổng quan, vị trí, tiện ích, mặt bằng và bảng giá chính thức Simona Heights.",
        isPublic: true,
        isActive: true,
      },
    });
    console.log("✅ Updated existing ProjectResource URL to:", targetMicrositeUrl);
  } else {
    ...
    console.log("✅ Created ProjectResource URL:", targetMicrositeUrl);
  }

  console.log("=== ALL DONE! SIMONA HEIGHTS WEBSITE IS READY & LINKED ===");
}

fixSimonaWebsite().finally(() => prisma.$disconnect());

...
async function main() {
  for (const item of allProjectsToTest) {
    ...
    console.log(`[${item.tag}] ${p.name} (/du-an/${p.slug}) | WS: ${p.website ? p.website.status : 'AUTO-GENERATED FALLBACK'} | Inv:${p.inventories.length} List:${p.listings.length} Res:${p.resources.length} 360:${!!tour360} | Active: ${activeSections.join(',')} | Hidden: ${hiddenSections.join(',') || 'None'}`);
  }
}
